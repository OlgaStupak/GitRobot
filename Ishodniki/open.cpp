...
double detectAngle(int xB, int xR, int yB, int yR)
{
    ...
    if (yB - yR == 0)
        return 90;
    //вычисл€ем угол, если если есть наклон
...
{
    // ѕолучаем H S V парметры
    int h = hsv[0];
    int s = hsv[1];
    int v = hsv[2];
    // ѕереводим H из формата [0, 360] в [2, 180]
    h = static_cast<int>(h * 180 / 360);
    // ѕереводим S и V из формата [0, 100] в [0, 255]
...
int main() {
    ...
    //в случае ошибки при открытии изображени€ с камеры прекращаем работу
    if (!cap.isOpened()) {
	...
	Scalar L_Blue = convert_hsv(Scalar(110, 50, 50));//нижн€€ граница задаваемого цвета
        ...
        inRange(hsv, L_Blue, U_Blue, Bmask);//провер€ем наличие цвета между заданными границами
        ...
        //дл€ завершени€ работы нажимаем ESC
